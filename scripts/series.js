...
function carregarTemporadas() {
    getDados(`/series/${serieId}/temporadas/todas`)
        .then(data => {
            const temporadasUnicas = [...new Set(data.map(numeroTemporada => numeroTemporada.numeroTemporada))];
            ...
            temporadasUnicas.forEach(numeroTemporada => {
                const option = document.createElement('option');
                option.value = numeroTemporada;
                option.textContent = numeroTemporada;
    ...
    getDados(`/series/${serieId}/temporadas/${listaTemporadas.value}`)
        .then(data => {
            ...
            temporadasUnicas.forEach(numeroTemporada => {
                ...
                const episodiosTemporadaAtual = data.filter(serie => serie.numeroTemporada === numeroTemporada);

                const listaHTML = episodiosTemporadaAtual.map(serie => `
                    <li>
                        ${serie.numeroEpisodio} - ${serie.tituloEpisodio}
                    </li>
                `).join('');
                ...
                paragrafo.textContent = `Temporada ${numeroTemporada}`;
...
// função para carregar os top episódios da série
function carregarTopEpisodios() {
    ...
    .then(data => {
        ...
        const listaHTML = data.map(serie =>`
            <li>
            Episódio ${serie.numeroEpisodio} - Temporada ${serie.numeroTemporada} - ${serie.tituloEpisodio}
            </li>
            `).join('');
            ul.innerHTML = listaHTML;
            const paragrafo = document.createElement('p');
